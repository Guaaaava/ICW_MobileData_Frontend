<template>
	<view class="container">
		...
		<view class="toolbar">
			<!-- 添加设置图标 -->
			      <u-icon name="settings" size="24" color="#333" @click="toggleSettings"></u-icon>
			      <u-select
			        v-model="show"
			        mode="single-column"
			    ...
			    <view v-if="settingsVisible" class="settings-sidebar">
			      <view class="settings-item">
			        <u-switch v-model="peakOn" @change="handlePeakChange"></u-switch>
			        <label>峰值</label>
			      </view>
			      <view class="settings-item">
			        <u-switch v-model="validOn" @change="handleValidChange"></u-switch>
			        <label>有效值</label>
			      </view>
			      <view class="settings-item">
			        <input type="number" v-model="thresholdValue" placeholder="输入阈值" />
			      </view>
			    </view>
...
let settingsVisible = ref(false); // 控制设置侧边框的显示
let peakOn = ref(true); // 峰值开关状态
let validOn = ref(true); // 有效值开关状态
let thresholdValue = ref(0.5); // 阈值输入
...
// 处理峰值开关变化
const handlePeakChange = (value) => {
  console.log('峰值开关状态：', value);
};

// 处理有效值开关变化
const handleValidChange = (value) => {
  console.log('有效值开关状态：', value);
};
</script>
